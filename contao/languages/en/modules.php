<?php

$GLOBALS['TL_LANG']['MOD']['issue_service_management'] = 'Service management';
$GLOBALS['TL_LANG']['MOD']['issue_service_dashboard'] = ['Dashboard', 'Show the issue service overview.'];
$GLOBALS['TL_LANG']['MOD']['issue_service_issues'] = ['Issues', 'Manage issues, comments and attachments.'];
$GLOBALS['TL_LANG']['MOD']['issue_service_services'] = ['Services', 'Manage services and categories.'];
$GLOBALS['TL_LANG']['MOD']['issue_service_categories'] = ['Categories', 'Manage ticket categories.'];
$GLOBALS['TL_LANG']['MOD']['issue_service_workflow'] = ['Workflow', 'Manage statuses and transitions.'];
$GLOBALS['TL_LANG']['MOD']['issue_service_settings'] = ['Settings', 'Manage issue service settings.'];