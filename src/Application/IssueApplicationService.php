...
{ 
    ...
    public function create(CreateIssueCommand $c):array
    {
        if(!$this->services->categoryBelongsToService($c->categoryId,$c->serviceId))
            throw new \InvalidArgumentException('Category does not belong to service.');
        ...
        return $db->transactional(
            function()use($c,$db)
            {
                $service=$db->fetchAssociative('SELECT alias FROM tl_issue_service WHERE id=:id AND published=1',['id'=>$c->serviceId]);
                ...
                $db->insert(
                    'tl_issue',[
                        ...
                        'guest_access_hash'=>$c->guestAccessHash,
                ...
                $db->insert(
                    'tl_issue_history',[
                        ...
                        'actor_type'=>$c->memberId?'member':'guest',
                        'actor_id'=>$c->memberId,
                        'new_value'=>json_encode(['ticket'=>$ticket],JSON_THROW_ON_ERROR),'created_at'=>$now
                    ]
                );
